  ...
  return (
    <>
      ...
      <p>
        ...
        <strong>electric currents were induced in closed coils when subjected to changing magnetic fields</strong>.
      </p>
      <KeyPoint title="Definition">
        The phenomenon in which electric current is generated by varying magnetic fields is appropriately called{" "}
        <Highlight color="yellow">electromagnetic induction</Highlight>.
      </KeyPoint>
      <Expandable title="Q: What is the use of it? — Faraday&apos;s famous reply">
        <p>When Faraday first made his discovery public — that relative motion between a bar magnet and a wire loop produced a small current — he was asked, &ldquo;What is the use of it?&rdquo;</p>
        <p>His reply: <Highlight color="yellow">&ldquo;What is the use of a new born baby?&rdquo;</Highlight></p>
      </Expandable>
      <Callout type="didyouknow">
        The phenomenon of electromagnetic induction is not merely of theoretical or academic interest — it is also of{" "}
        <Highlight color="yellow">practical utility</Highlight>. The pioneering experiments of{" "}
        <Highlight color="blue">Faraday and Henry</Highlight> have led directly to the development of modern day{" "}
        <strong>generators</strong> and <strong>transformers</strong>.
      </Callout>
      ...
      <Callout type="warning">
        <strong>Practical caution:</strong> sensitive electrical instruments in the vicinity of an electromagnet can be{" "}
        <strong>damaged due to induced emfs</strong> (and the resulting currents) when the electromagnet is turned on or off.
      </Callout>
      ...
      <p>If the field varies over the surface, we sum over area elements d<strong>A</strong><sub>i</sub>:</p>
      <FormulaBlock latex={String.raw`\Phi_B = \mathbf{B}_1 \cdot \mathrm{d}\mathbf{A}_1 + \mathbf{B}_2 \cdot \mathrm{d}\mathbf{A}_2 + \cdots = \sum_{\text{all } i} \mathbf{B}_i \cdot \mathrm{d}\mathbf{A}_i`} />
      <div className="grid gap-4 sm:grid-cols-2 [&>div]:my-0">
        <FactCard
          title="SI Unit"
          definition="The SI unit of magnetic flux is the weber (Wb) or tesla metre squared (T m²)."
          icon="📏"
        />
        <FactCard
          title="Nature"
          definition="Magnetic flux is a scalar quantity — the dot product of two vectors B and A."
          icon="🔢"
        />
      </div>
      ...
      <Callout type="important">
        ...
        <strong>change the magnetic flux</strong> through the coil — and it is this change that induces the emf.
      </Callout>
      ...
      <p>
        From Eqs. (6.1) and (6.2), the flux can be varied by changing <strong>any one or more of the terms B, A and θ</strong>. In Experiments 6.1
        and 6.2 the flux changes by varying <strong>B</strong>. The flux can also change by{" "}
        <strong>changing the shape of a coil</strong> (shrinking or stretching it) in a magnetic field, or by{" "}
        <strong>rotating a coil</strong> so that the angle θ between <strong>B</strong> and <strong>A</strong> changes.
      </p>
      ...
      <p>In 1834, the German physicist <strong>Heinrich Friedrich Lenz (1804–1865)</strong> deduced a rule which gives the polarity of the induced emf in a clear and concise fashion.</p>
      <Callout type="important">
      ...
      />
      <Expandable title="Q: Why can&apos;t the induced current ever assist the motion? (Perpetual motion)">
        <p>Suppose the induced current were in the <em>opposite</em> direction to that given by Lenz&apos;s law. In Fig. 6.6(a), the south-pole of the induced current would face the approaching north-pole of the magnet — the bar magnet would be <strong>attracted</strong> at ever-increasing acceleration.</p>
        <p>A gentle push would initiate the process and the magnet&apos;s <strong>velocity and kinetic energy would increase without any energy being spent</strong> — a <Highlight color="pink">perpetual-motion machine</Highlight>. This violates the law of conservation of energy, and hence cannot happen.</p>
      </Expandable>
      <p>In the correct case (repulsion), a person must do <strong>work in moving the magnet</strong>. Where does this energy go?</p>
