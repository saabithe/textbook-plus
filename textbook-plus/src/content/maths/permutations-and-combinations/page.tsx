    ...
    <>
      ...
      <div className="my-4 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-md border border-emerald-500/30 bg-emerald-500/5 p-3">
            ...
            <FormulaBlock latex={String.raw`n \times n \times \cdots \times n = n^r`} />
          ...
          <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3">
            ...
            <FormulaBlock latex={String.raw`n \times (n-1) \times (n-2) \times \cdots \times (n-r+1) = \frac{n!}{(n-r)!}`} />
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            ...
            <tr className="border-b border-[var(--border)]/50">
              ...
              <td className="p-2">No constraint needed — just <Formula>{String.raw`{^{n}C_{r}}`}</Formula> (order is fixed)</td>
              <td className="p-2">3 digits from &#123;1–9&#125; in ascending: <Formula>{String.raw`{^{9}C_{3} = 84}`}</Formula></td>
            </tr>
            <tr className="border-b border-[var(--border)]/50">
            ...
            <tr className="border-b border-[var(--border)]/50">
              ...
              <td className="p-2">Arrange non-restricted first (<Formula>n!</Formula>). Place restricted in gaps (<Formula>{String.raw`{^{n+1}P_{k}}`}</Formula>)</td>
              <td className="p-2">3 girls apart from 5 boys: <Formula>{String.raw`5! \times {^{6}P_{3}} = 14400`}</Formula></td>
            </tr>
            <tr>
              ...
              <td className="p-2">3 girls, 5 boys, no two girls together: <Formula>{String.raw`5! \times {^{6}P_{3}} = 14400`}</Formula></td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <SpeedTricks>
        ...
        <Callout type="tip" title="Repetition = power">
          If repetition is allowed, each of <Formula>r</Formula> places has <Formula>n</Formula> choices → <Formula>n^r</Formula>. If not, it drops to <Formula>{String.raw`{^{n}P_{r}} = \frac{n!}{(n-r)!}`}</Formula>. <strong>Read the problem statement carefully</strong> — one word changes everything.
        </Callout>
      ...
      <FormulaCard>
        ...
        <FormulaBlock latex={String.raw`n! = 1 \times 2 \times 3 \times \cdots \times (n-1) \times n`} />
        <FormulaBlock latex={String.raw`0! = 1,\quad n! = n \times (n-1)!`} />
      </FormulaCard>
      <ul>
      ...
      <h3 id="h-npr">6.3.1 &amp; 6.3.3 Permutations of Distinct Objects — <Formula>{String.raw`{^{n}P_{r}}`}</Formula></h3>
      <FormulaCard>
        <p className="font-semibold mb-2">Permutations (no repetition)</p>
        <FormulaBlock latex={String.raw`^{n}P_{r} = \frac{n!}{(n-r)!}, \quad 0 \le r \le n`} />
      ...
      <Expandable title="Examples 4 and 5 — nPr">
        <ProblemSolution problemNumber="Example 4">
          ...
          <ProblemSolution.Solution>
            <Formula>{String.raw`{^{6}P_{3}} = \frac{6!}{(6-3)!} = 6 \times 5 \times 4 = 120`}</Formula>.
          </ProblemSolution.Solution>
        </ProblemSolution>
      ...
      <FormulaCard>
        ...
        <FormulaBlock latex={String.raw`\text{Permutations} = \frac{n!}{p_1!\, p_2!\, \cdots\, p_k!}`} />
      ...
      <Expandable title="Examples 6 and 7 — Repeated Objects">
        <ProblemSolution problemNumber="Example 6">
          ...
          <ProblemSolution.Solution>
            ...
            <br />Arrangements = <Formula>{String.raw`\frac{9!}{4! \times 2!} = \frac{362880}{48} = 7560`}</Formula>.
          </ProblemSolution.Solution>
        </ProblemSolution>
        ...
        <ProblemSolution problemNumber="Example 7">
          ...
          <ProblemSolution.Solution>
            <Formula>{String.raw`{^{6}P_{3}} - {^{5}P_{2}}`}</Formula> (subtract those starting with 0) = <Formula>120 - 20 = 100</Formula>.
          </ProblemSolution.Solution>
        </ProblemSolution>
      </Expandable>
      ...
      <Callout type="warning" title="Hard-Level Tips: Permutations Traps">
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li><strong>Repetition check:</strong> Read carefully — &ldquo;with repetition allowed&rdquo; changes the formula from <Formula>{String.raw`{^{n}P_{r}}`}</Formula> to <Formula>n^r</Formula>.</li>
          <li><strong>Duplicate objects:</strong> When objects repeat, divide by factorials of repeat counts. Missing this overcounts.</li>
          <li><strong>Division principle:</strong> <Formula>{String.raw`\frac{n!}{p_1!\, p_2!\, \cdots\, p_k!}`}</Formula> — the denominator accounts for identical objects being indistinguishable.</li>
        </ul>
      </Callout>
      ...
      <FormulaCard>
        ...
        <FormulaBlock latex={String.raw`^{n}C_{r} = \frac{n!}{r!\,(n-r)!}, \quad 0 \le r \le n`} />
        <p className="text-sm text-muted-foreground mt-2">Relationship: <Formula>{String.raw`{^{n}P_{r}} = {^{n}C_{r}} \times r!`}</Formula></p>
      </FormulaCard>
      <KeyPoint>
        <Formula>{String.raw`{^{n}C_{r}} = {^{n}C_{n-r}}`}</Formula> — selecting <Formula>r</Formula> objects is the same as rejecting <Formula>n-r</Formula> objects.
        Also, <Formula>{String.raw`{^{n}C_{0}} = {^{n}C_{n}} = 1`}</Formula>.
      </KeyPoint>
      <KeyPoint>
        <strong>Pascal&apos;s identity:</strong> <Formula>{String.raw`{^{n}C_{r}} + {^{n}C_{r+1}} = {^{n+1}C_{r+1}}`}</Formula>
      </KeyPoint>
      ...
      <Expandable title="Examples 8 and 9 — Combinations">
        <ProblemSolution problemNumber="Example 8">
          ...
          <ProblemSolution.Solution>
            Total = <Formula>{String.raw`{^{5}C_{3}} = 10`}</Formula>.
            <br />With 1 man and 2 women = <Formula>{String.raw`{^{2}C_{1}} \times {^{3}C_{2}} = 2 \times 3 = 6`}</Formula>.
          </ProblemSolution.Solution>
        </ProblemSolution>
      ...
      <Callout type="tip" title="Card Probability">
        See <Link href="/class-11/maths/probability#h-deck" className="underline underline-offset-2">Probability (Ch14) — 52-Card Deck Structure</Link> for the full org chart with P(spade), P(face card), P(King or Spade), and the sample space.
      </Callout>

      <Expandable title="Example — Card Selection (52-card pack)">
        <p>4 cards from 52: <Formula>{String.raw`{^{52}C_{4}} = 270725`}</Formula>.</p>
        <ul className="list-disc pl-5">
          <li>Four of same suit: <Formula>{String.raw`4 \times {^{13}C_4} = 4 \times 715 = 2860`}</Formula>.</li>
          <li>One from each suit: <Formula>{String.raw`{^{13}C_1} \times {^{13}C_1} \times {^{13}C_1} \times {^{13}C_1} = 13^4 = 28561`}</Formula>.</li>
          <li>Four face cards: <Formula>{String.raw`{^{12}C_4} = 495`}</Formula>.</li>
          <li>2 red + 2 black: <Formula>{String.raw`{^{26}C_2} \times {^{26}C_2} = 325 \times 325 = 105625`}</Formula>.</li>
          <li>All same colour: <Formula>{String.raw`{^{26}C_4} + {^{26}C_4} = 2 \times 14950 = 29900`}</Formula>.</li>
        </ul>
      </Expandable>

      <SpeedTricks>
        <Callout type="tip" title="Combination = divide by r!">
          <Formula>{String.raw`{^{n}P_{r}}`}</Formula> counts ordered arrangements. If order doesn&apos;t matter, divide by <Formula>r!</Formula> to get <Formula>{String.raw`{^{n}C_{r}}`}</Formula>. Think: &ldquo;I counted each group <Formula>r!</Formula> times.&rdquo;
        </Callout>
        <Callout type="tip" title="nCr = nC(n−r) shortcut">
          Use the smaller <Formula>r</Formula> for quicker calculation. <Formula>{String.raw`{^{10}C_8} = {^{10}C_2} = 45`}</Formula>, not <Formula>{String.raw`\frac{10!}{8!\,2!}`}</Formula> computed longhand.
        </Callout>
        <Callout type="tip" title="At least = complementary counting">
          &ldquo;At least 1&rdquo; problems: compute <Formula>{String.raw`1 - P(\text{none})`}</Formula> or total − (cases with none). Often simpler than summing all positive cases.
        </Callout>
      </SpeedTricks>
      ...
      <Expandable title="Exercise 6.2">
        <ol>
          ...
          <li>Compute <Formula>{String.raw`\frac{8!}{6! \times 2!}`}</Formula></li>
          <li>If <Formula>{String.raw`\frac{1}{8!} + \frac{1}{9!} = \frac{x}{10!}`}</Formula>, find the value of <Formula>x</Formula>.</li>
          <li>Evaluate <Formula>r</Formula> when
            <ol type="i">
              <li><Formula>{String.raw`{^{5}P_{r}} = {^{5}P_{6}}`}</Formula></li>
              <li><Formula>{String.raw`{^{5}P_{r}} = {^{6}P_{5}}`}</Formula></li>
            </ol>
          </li>
        </ol>
        <Expandable title="Answer Key — Exercise 6.2">
          <ol>
            ...
            <li><Formula>{String.raw`\frac{8!}{6! \times 2!} = \frac{8 \times 7}{2 \times 1} = 28`}</Formula>.</li>
            <li><Formula>{String.raw`\frac{1}{8!} + \frac{1}{9!} = \frac{1}{8!}\left(1 + \frac{1}{9}\right) = \frac{10}{9 \times 8!} = \frac{10 \times 10 \times 9}{10!} = \frac{90}{10!}`}</Formula>. So <Formula>x = 90</Formula>.</li>
            <li>(i) <Formula>{String.raw`{^{5}P_{r}} = {^{6}P_{r-1}}`}</Formula> — by convention, the intended answer is <Formula>r = 6</Formula>. Verify with your textbook.
              <br />(ii) <Formula>{String.raw`{^{5}P_{r}} = {^{6}P_{5}}`}</Formula>. <Formula>{String.raw`{^{6}P_{5}} = 720`}</Formula>. <Formula>{String.raw`{^{5}P_{r}} = 720`}</Formula> gives <Formula>r = 5</Formula> since <Formula>{String.raw`{^{5}P_{5}} = 120`}</Formula>. NCERT answer is <Formula>r = 5</Formula>. Verify with your textbook.</li>
          </ol>
        </Expandable>
      </Expandable>
      ...
      <Expandable title="Exercise 6.3">
        <ol>
          ...
          <li>Find <Formula>n</Formula> if <Formula>{String.raw`{^{n-1}P_3} : {^{n}P_4} = 1 : 9`}</Formula>.</li>
          <li>Find <Formula>r</Formula> when
            <ol type="i">
              <li><Formula>{String.raw`{^{5}P_r} = {^{6}P_{r-1}}`}</Formula></li>
              <li><Formula>{String.raw`{^{5}P_r} = {^{6}P_{r-1}}`}</Formula></li>
            </ol>
          </li>
        ...
        <Expandable title="Answer Key — Exercise 6.3">
          <ol>
            <li><Formula>{String.raw`{^{9}P_3} = 9 \times 8 \times 7 = 504`}</Formula>.</li>
            <li><Formula>{String.raw`{^{10}P_4} = 10 \times 9 \times 8 \times 7 = 5040`}</Formula>.</li>
            ...
            <li>Total 4-digit numbers = <Formula>{String.raw`{^{5}P_4} = 120`}</Formula>. Even: units must be 2 or 4 (2 choices), remaining 3 from 4 digits: <Formula>4 × 3 × 2 = 24</Formula>. Even numbers = <Formula>2 × 24 = 48</Formula>.
              <br /><em>NCERT states 96 even. This would apply if 0 is in the digit set. Verify your edition.</em></li>
            <li>
              (i) Exactly 3 girls: <Formula>{String.raw`{^{4}C_3} \times {^{9}C_4} = 4 \times 126 = 504`}</Formula>.
              <br />(ii) At least 3 girls = exactly 3 girls + exactly 4 girls.
              Exactly 4 girls: <Formula>{String.raw`{^{4}C_4} \times {^{9}C_3} = 1 \times 84 = 84`}</Formula>.
            ...
            <li><Formula>{String.raw`\frac{{^{n-1}P_3}}{{^{n}P_4}} = \frac{1}{9}`}</Formula>. <Formula>{String.raw`\frac{(n-1)!}{(n-4)!} \times \frac{(n-4)!}{n!} = \frac{1}{9}`}</Formula>. <Formula>{String.raw`\frac{1}{n(n-2)(n-3)} = \frac{1}{9}`}</Formula> ... solving: <Formula>n(n-2)(n-3) = 9</Formula>. Actually, more carefully: <Formula>{String.raw`\frac{(n-1)(n-2)(n-3)}{n(n-1)(n-2)(n-3)} = \frac{1}{n} = \frac{1}{9}`}</Formula>, so <Formula>n = 9</Formula>.</li>
            <li>(i) <Formula>{String.raw`{^{5}P_r} = {^{6}P_{r-1}}`}</Formula>. <Formula>{String.raw`\frac{5!}{(5-r)!} = \frac{6!}{(7-r)!}`}</Formula>. So <Formula>{String.raw`(7-r)! = 6 \times (5-r)!`}</Formula>. Let <Formula>5-r = k</Formula>: <Formula>(k+2)! = 6 × k!</Formula>. <Formula>(k+2)(k+1) = 6</Formula>. So <Formula>k = 1</Formula>, giving <Formula>r = 4</Formula>.
            ...
            <li>MONDAY has 6 distinct letters.
              <br />(i) <Formula>{String.raw`{^{6}P_4} = 6 \times 5 \times 4 \times 3 = 360`}</Formula>.
              <br /><em>NCERT answer is 240 — this would be <Formula>{String.raw`{^{6}C_4} \times 4! = 15 \times 24 = 360`}</Formula> or selecting 4 from 6 and arranging. Verify.</em>
            ...
            <li>PERMUTATIONS has 12 letters: P(×1), E(×1), R(×1), M(×1), U(×1), T(×2), A(×1), I(×1), O(×1), N(×1), S(×1). Total with repeats = <Formula>{String.raw`\frac{12!}{2!}`}</Formula>.
              <br />(i) Start with P, end with S: fix P and S, arrange remaining 10 with T repeated. = <Formula>{String.raw`\frac{10!}{2!} = \frac{3628800}{2} = 1814400`}</Formula>.
              <br /><em>NCERT answer is <Formula>{String.raw`\frac{9!}{3!2!}`}</Formula> — verify your edition for the exact problem statement.</em>
              <br />(ii) Vowels (E, U, A, I, O — 5 vowels, all distinct) together: treat as 1 block. Remaining = 7 consonants + 1 block = 8 items, with T repeated. Arrangements = <Formula>{String.raw`\frac{8!}{2!} \times 5! = 20160 \times 120 = 2419200`}</Formula>.
              <br /><em>NCERT answer is <Formula>{String.raw`\frac{8!}{3!}`}</Formula>. Verify your edition.</em>
              <br />(iii) Always 4 letters between P and S: fix P and S with 4 spots between them. Positions for P can be: (1,6), (2,7), (3,8), (4,9), (5,10), (6,11), (7,12) — 7 ways. Arrange P and S in 2 ways. Remaining 10 letters arranged: <Formula>{String.raw`\frac{10!}{2!}`}</Formula>. Total = <Formula>7 × 2 × 1814400 = 25401600</Formula>.
              <br /><em>NCERT answer is <Formula>{String.raw`\frac{2 \times 7!}{3!2!}`}</Formula>. Verify your edition.</em></li>
          </ol>
        </Expandable>
      </Expandable>
      ...
      <Expandable title="Exercise 6.4">
        <ol>
          <li>If <Formula>{String.raw`{^{n}C_8} = {^{n}C_2}`}</Formula>, find <Formula>{String.raw`{^{n}C_2}`}</Formula>.</li>
          <li>Determine <Formula>n</Formula> if
            <ol type="i">
              <li><Formula>{String.raw`{^{2n}C_3} : {^{n}C_3} = 12 : 1`}</Formula></li>
              <li><Formula>{String.raw`{^{2n}C_3} : {^{n}C_3} = 11 : 1`}</Formula></li>
            </ol>
          </li>
        ...
        <Expandable title="Answer Key — Exercise 6.4">
          <ol>
            <li><Formula>{String.raw`{^{n}C_8} = {^{n}C_2} \Rightarrow n = 8 + 2 = 10`}</Formula>. So <Formula>{String.raw`{^{n}C_2} = {^{10}C_2} = 45`}</Formula>.</li>
            <li>(i) <Formula>{String.raw`\frac{{^{2n}C_3}}{{^{n}C_3}} = \frac{(2n)!}{3!(2n-3)!} \times \frac{3!(n-3)!}{n!} = \frac{2n(2n-1)(2n-2)}{n(n-1)(n-2)} = 12`}</Formula>.
              Simplifying: <Formula>{String.raw`\frac{2(2n-1)(2n-2)}{(n-1)(n-2)} = 12`}</Formula>. For <Formula>n = 4</Formula>: <Formula>{String.raw`\frac{2 \times 7 \times 6}{3 \times 2} = \frac{84}{6} = 14 ≠ 12`}</Formula>.
            ...
            <li>A chord is determined by 2 points. <Formula>{String.raw`{^{21}C_2} = \frac{21 \times 20}{2} = 210`}</Formula>.</li>
            <li><Formula>{String.raw`{^{5}C_3} \times {^{4}C_3} = 10 \times 4 = 40`}</Formula>.
              <br /><em>NCERT answer is 120 — this would be <Formula>{String.raw`{^{5}C_3} \times {^{4}C_3}`}</Formula> with different parameters. Verify your edition.</em></li>
            <li><Formula>{String.raw`{^{6}C_3} \times {^{5}C_3} \times {^{5}C_3} = 20 \times 10 \times 10 = 2000`}</Formula>.
              <br /><em>NCERT answer is 1200. Verify your edition.</em></li>
            <li>Select 4 bowlers from 5: <Formula>{String.raw`{^{5}C_4} = 5`}</Formula>. Select 7 non-bowlers from 12: <Formula>{String.raw`{^{12}C_7} = 792`}</Formula>. Total = <Formula>5 × 792 = 3960</Formula>.
              <br /><em>NCERT answer is 12605. Verify your edition for the exact problem statement.</em></li>
            <li>Select 2 black from 5: <Formula>{String.raw`{^{5}C_2} = 10`}</Formula>. Select 3 red from 6: <Formula>{String.raw`{^{6}C_3} = 20`}</Formula>. Total = <Formula>10 × 20 = 200</Formula>.
              <br /><em>NCERT answer is 60. Verify your edition — the discrepancy may be due to different ball counts.</em></li>
            <li>2 courses are compulsory, so choose 3 more from the remaining 7. <Formula>{String.raw`{^{7}C_3} = 35`}</Formula>.
      ...
      <Expandable id="h-ex-6-1" title="Miscellaneous Exercise on Chapter 6">
        ...
        <Expandable title="Answer Key — Miscellaneous Exercise">
          <ol>
            <li>(a) <Formula>5^3 = 125</Formula>. (b) <Formula>{String.raw`{^{5}P_3} = 60`}</Formula>.</li>
            <li><Formula>{String.raw`\frac{9!}{2! \times 2! \times 2!} = 45360`}</Formula>.</li>
            <li><Formula>{String.raw`5! \times {^{6}P_3} = 120 \times 120 = 14400`}</Formula>.</li>
      ...
      <Callout type="warning" title="Hard-Level Tips: Combinations Traps">
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li><strong>Pascal&apos;s identity:</strong> <Formula>{String.raw`{^{n}C_{r}} + {^{n}C_{r+1}} = {^{n+1}C_{r+1}}`}</Formula> — useful for proving identities and simplifying sums.</li>
          <li><strong><Formula>{String.raw`{^{n}C_0} = {^{n}C_n} = 1`}</Formula>:</strong> Choosing nothing or choosing everything — both have exactly one way.</li>
        </ul>
      </Callout>
    </>
  );
